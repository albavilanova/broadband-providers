...
// Show available products by condition
    ...
    if (conditions) {
        ...
        if (conditions.hasOwnProperty("variables")) {
            variablesArray = conditions["variables"].split(',').map((str) => str.trim());
        }
        ...
        const products = await db.product.findMany({
            where: {
                ...
                provider: provider,
                variables: variablesArray.length >0 ? ({ hasEvery: variablesArray }) : undefined
            }
        });
...
// Return error if less than 2 or more than 5 arguments have been passed
    ...
    console.error("Usage: bun product.ts --name=<name> --providerName=<providerName>  --variables=<variables>");
    process.exit(1);
}